...
class ControladorFacturas{
	...
	static public function ctrMostrarCortes($item, $valor, $orden, $optimizacion){

		$tabla = "cortes_caja";

		if($optimizacion == "si"){
			$respuesta = ModeloFacturas::MdlMostrarCortesOptimizados($tabla, $item, $valor, $orden);
		} else {
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoClienteFactura"])){
				...
				if($_POST["tipoDte"] == "05" || $_POST["tipoDte"] == "06"){
					...
					foreach ($productos as $producto) {
						...
						$productoTraido = ControladorProductos::ctrMostrarProductos($item, $valor, "no");
					...
					if($respuesta != "error"){
						...
						swal({
							...
							allowOutsideClick: false
								 ...
								 window.location = "index.php?ruta=facturacion&idFacturaRapida=' . $respuesta . '";
	...
	static public function ctrEnviarFacturaCorreo(){
		...
		if (isset($_GET["idFacturaEnviarCorreo"])) {
			...
			try {
				// URL del PDF
				$pdfUrl = "http://localhost/LIBRERIA-NUEVO-MUNDO/FOX-CONTROL/extensiones/TCPDF-main/examples/imprimir-factura.php?idFactura={$_GET['idFacturaEnviarCorreo']}";
	...
	private static function enviarCorreo($from, $to, $subject, $message, $pdfUrl, $codigoGenera, $jsonContent){
		...
		$mail->Username = $from;
		$mail->Password = 'nuevomundo_'; // Usa variables seguras para contraseñas
		$mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
		$mail->Port = 465;

		$mail->setFrom($from, 'Librería Nuevo Mundo');
		$mail->addAddress($to);
		...
		$mail->Body = $message;
		 ...
		 try {
			if ($pdfUrl) {
			 ...
			 $mail->addAttachment($jsonFilePath, 'FacturaData.json');  // Adjuntar el archivo JSON
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevaFormaAbono"])){
				...
				$datos = array("id_factura" => $factura["id"],
							   ...
							   "monto" => $_POST["nuevoMontoAbono"],
								"dinero_cliente" => $_POST["nuevoMontoDinero"]);
				...
				if($respuesta == "ok"){
					...
					swal({
						...
						window.location = "index.php?ruta=ver-factura&idFacturaEditar='.$factura["id"].'&pagado=si";
	...
	static public function ctrCrearCorteCaja(){
		...
		if(isset($_GET["crearCorte"])){
			...
			if(isset($_GET["crearCorte"])){
				...
				date_default_timezone_set('America/El_Salvador');   // ¡importante!

				$raw = filter_input(INPUT_GET, 'fecha', FILTER_SANITIZE_STRING);

				if ($raw) {
					// ¿incluye hora (longitud > 10 o tiene espacio/T)?
					if (strlen($raw) > 10) {
						$fechaFormateada = date('Y-m-d H:i:s', strtotime($raw)); // normalizas formato
					} else {
						// Solo YYYY-MM-DD
						$hoy = date('Y-m-d');
						$hora = ($raw === $hoy) ? date('H:i:s') : '23:59:59';  // o '00:00:00'
						$fechaFormateada = "$raw $hora";
					}
				} else {
					// No vino parámetro: usas fecha y hora actuales
					$fechaFormateada = date('Y-m-d H:i:s');
				}

	
				$item = "fecEmi";
				$valor  = substr($fechaFormateada, 0, 10);
				$orden = "id";
				$optimizacion = "no";
